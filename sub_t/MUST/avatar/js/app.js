...
// ============================================
// INTEGRACIÓN CON MUSTQUEST
// ============================================

$(document).ready(function() {
    // Cargar avatar guardado de MustQuest si existe
    loadMustQuestAvatar();
    
    // Botón para guardar en MustQuest
    $('#save-mustquest-avatar').click(function() {
        saveMustQuestAvatar();
    });
    ...
    // Botón GUARDAR Y VOLVER AL JUEGO
    $('#save-and-return').on('click', function() {
        // Guardar SVG del avatar en localStorage
        const avatarSvg = $(".viewer").html();
        localStorage.setItem('mustQuestAvatarSvg', avatarSvg);
        localStorage.setItem('mustQuestAvatar', JSON.stringify({
            created: new Date().toISOString(),
            type: 'advanced-cartoon',
            data: avatarSvg
        }));
        // Mensaje de éxito y volver
        Swal.fire({
            title: '¡Avatar guardado!',
            text: 'Tu avatar ha sido guardado exitosamente.',
            icon: 'success',
            timer: 1500,
    ...
    // Botón DESCARGAR AVATAR
    $('#download').on('click', function() {
        // Descargar SVG como archivo
        const svgElement = document.getElementById('character');
        if (!svgElement) {
            Swal.fire('Error', 'No se encontró el avatar para descargar.', 'error');
            return;
        }
    ...
    $('#return-without-saving').on('click', function() {
        Swal.fire({
            title: '¿Salir sin guardar?',
            text: 'Perderás los cambios realizados en tu avatar.',
            icon: 'warning',
            showCancelButton: true,
    ...
    // Modificar función updateStorage para que también guarde en MustQuest
    const originalUpdateStorage = window.updateStorage;
    window.updateStorage = function() {
        // Llamar función original
        if (originalUpdateStorage) {
            originalUpdateStorage();
        }
        
        // Guardar también en formato MustQuest
        const avatarSvg = $(".viewer").html();
        localStorage.setItem('mustQuestAvatarSvg', avatarSvg);
        localStorage.setItem('mustQuestAvatar', JSON.stringify({
            created: new Date().toISOString(),
...
function loadMustQuestAvatar() {
    // Cargar avatar previamente guardado de MustQuest
    const savedAvatar = localStorage.getItem('CartoonAvatarCreator');
    if (savedAvatar) {
        try {
            const avatarData = JSON.parse(savedAvatar);
            if (avatarData.design) {
                $(".viewer").html(avatarData.design);
            }
        } catch (e) {
            console.log('No se pudo cargar avatar guardado:', e);
        }
    }
}

function saveMustQuestAvatar() {
    // Obtener el SVG del avatar
    const avatarSvg = $(".viewer").html();
    
    // Guardar en localStorage con las claves de MustQuest
    localStorage.setItem('mustQuestAvatarSvg', avatarSvg);
    localStorage.setItem('mustQuestAvatar', JSON.stringify({
        created: new Date().toISOString(),
        type: 'advanced-cartoon',
    ...
    swal({
        title: '¡Avatar Guardado!',
        text: 'Tu avatar ha sido guardado exitosamente en MustQuest',
        icon: 'success',
        timer: 2000,
        showConfirmButton: false
    }).then(() => {
